...
impl Engine {
    ...
    pub fn evaluate_expression(&mut self, expression: &str, variables: JsValue) -> Result<f64, JsValue> {
        ...
        // Execute formula
        self.inner.execute(vec![formula])
            .map_err(|e| JsValue::from_str(&format!("Execution error: {}", e)))?;
        
        // Get result
        let result = self.inner.get_result("_temp")
...
impl Formula {
    ...
    pub fn parse(expression: &str) -> Result<Formula, JsValue> {
        ...
        // Test if it's valid
        let mut test_engine = CoreEngine::new();
        test_engine.execute(vec![formula])
