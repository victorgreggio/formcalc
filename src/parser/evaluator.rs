        ...
        match stmt {
            ...
            Statement::If {
                ...
                if let Some(else_blk) = else_block {
                    ...
                    Err(CalculatorError::EvalError("No matching condition".to_string()))
                }
            }
    ...
    fn evaluate_expr(&self, expr: &Expr) -> Result<Value> {
        match expr {
            ...
            Expr::Subtract(left, right) => {
                ...
                match (l, r) {
                    (Value::Number(a), Value::Number(b)) => Ok(Value::Number(a - b)),
                    _ => Err(CalculatorError::TypeError("Subtraction requires numbers".to_string())),
                }
            }
            Expr::Multiply(left, right) => {
                ...
                match (l, r) {
                    (Value::Number(a), Value::Number(b)) => Ok(Value::Number(a * b)),
                    _ => Err(CalculatorError::TypeError("Multiplication requires numbers".to_string())),
                }
            }
            Expr::Divide(left, right) => {
                ...
                match (l, r) {
                    ...
                    _ => Err(CalculatorError::TypeError("Division requires numbers".to_string())),
                }
            }
            Expr::Power(left, right) => {
                ...
                match (l, r) {
                    ...
                    _ => Err(CalculatorError::TypeError("Power requires numbers".to_string())),
                }
            }
            Expr::Modulo(left, right) => {
                ...
                match (l, r) {
                    (Value::Number(a), Value::Number(b)) => Ok(Value::Number(a % b)),
                    _ => Err(CalculatorError::TypeError("Modulo requires numbers".to_string())),
                }
            }
            ...
            Expr::LessThan(left, right) => {
                ...
                match l.partial_cmp(&r) {
                    ...
                    None => Err(CalculatorError::TypeError("Cannot compare values of different types".to_string())),
                }
            }
            ...
            Expr::And(left, right) => {
                ...
                match (l, r) {
                    ...
                    _ => Err(CalculatorError::TypeError("Logical AND requires booleans".to_string())),
                }
            }
            Expr::Or(left, right) => {
                let l = self.evaluate_expr(left)?;
                ...
                match (l, r) {
                    (Value::Bool(a), Value::Bool(b)) => Ok(Value::Bool(a || b)),
                    _ => Err(CalculatorError::TypeError("Logical OR requires booleans".to_string())),
                }
            }
            Expr::Not(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Logical NOT requires boolean".to_string())),
                }
            }
            ...
            Expr::UnaryMinus(expr) => {
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Unary minus requires number".to_string())),
                }
            }
            ...
            Expr::Max(left, right) => {
                ...
                match (l, r) {
                    ...
                    _ => Err(CalculatorError::TypeError("Max requires numbers".to_string())),
                }
            }
            Expr::Min(left, right) => {
                let l = self.evaluate_expr(left)?;
                ...
                match (l, r) {
                    ...
                    _ => Err(CalculatorError::TypeError("Min requires numbers".to_string())),
                }
            }
            Expr::Rnd(left, right) => {
                let l = self.evaluate_expr(left)?;
                ...
                match (l, r) {
                    ...
                    _ => Err(CalculatorError::TypeError("Rnd requires numbers".to_string())),
                }
            }
            Expr::Ceil(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Ceil requires number".to_string())),
                }
            }
            Expr::Floor(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Floor requires number".to_string())),
                }
            }
            Expr::Exp(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Exp requires number".to_string())),
                }
            }
            Expr::Year(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Year requires string date".to_string())),
                }
            }
            Expr::Month(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Month requires string date".to_string())),
                }
            }
            Expr::Day(expr) => {
                let val = self.evaluate_expr(expr)?;
                ...
                match val {
                    ...
                    _ => Err(CalculatorError::TypeError("Day requires string date".to_string())),
                }
            }
            Expr::Substr(str_expr, start_expr, len_expr) => {
                ...
                match (s, start, len) {
                    ...
                    _ => Err(CalculatorError::TypeError("Substr requires (string, number, number)".to_string())),
                }
            }
            Expr::AddDays(date_expr, days_expr) => {
                ...
                match (date_val, days_val) {
                    (Value::String(s), Value::Number(days)) => {
                        ...
                        Ok(Value::String(new_date.format("%Y-%m-%dT%H:%M:%S").to_string()))
                    }
                    _ => Err(CalculatorError::TypeError("AddDays requires (string date, number)".to_string())),
                }
            }
            Expr::GetDiffDays(date1_expr, date2_expr) => {
                ...
                match (date1_val, date2_val) {
                    ...
                    _ => Err(CalculatorError::TypeError("GetDiffDays requires two string dates".to_string())),
                }
            }
            Expr::PaddedString(str_expr, width_expr) => {
                ...
                match (s, width) {
                    ...
                    _ => Err(CalculatorError::TypeError("PaddedString requires (string, number)".to_string())),
                }
            }
            Expr::DifferenceInMonths(date1_expr, date2_expr) => {
                ...
                match (date1_val, date2_val) {
                    (Value::String(s1), Value::String(s2)) => {
                        let date1 = parse_date(&s1)?;
                        let date2 = parse_date(&s2)?;
                        let months = (date1.year() - date2.year()) * 12 + (date1.month() as i32 - date2.month() as i32);
                        Ok(Value::Number(months.abs() as f64))
                    }
                    _ => Err(CalculatorError::TypeError("DifferenceInMonths requires two string dates".to_string())),
                }
            }
                ...
                match formula_name {
                    Value::String(name) => {
                        self.formula_result_cache
                            .get(&name)
                            .ok_or_else(|| CalculatorError::FormulaNotFound(name))
                    }
                    _ => Err(CalculatorError::TypeError("GetOutputFrom requires string".to_string())),
                }
            }
...
fn parse_date(s: &str) -> Result<NaiveDateTime> {
    NaiveDateTime::parse_from_str(s, "%Y-%m-%dT%H:%M:%S")
        ...
        .map_err(|e| CalculatorError::DateParseError(format!("Failed to parse date '{}': {}", s, e)))
}
